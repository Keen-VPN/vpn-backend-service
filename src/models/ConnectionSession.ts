...
type PrismaConnectionSession = NonNullable<
  Awaited<ReturnType<typeof prisma.connectionSession.findUnique>>
>;
...
class ConnectionSession {
   ...
   */
  async create(
    sessionData: CreateConnectionSessionData
  ...
  async getStats(userId: string): Promise<ConnectionStats> {
    try {
      ...
      const totalDuration = sessions.reduce(
        (sum: number, s: SessionData) => sum + s.durationSeconds,
        0
      );
      const totalBytes = sessions.reduce(
        (sum: number, s: SessionData) => sum + Number(s.bytesTransferred || 0),
        0
      );
      ...
      // Group by platform
      const platformStats = sessions.reduce(
        (
          acc: Record<
            string,
            { sessions: number; duration: number; bytes: number }
          >,
          session: SessionData
        ) => {
  ...
  async deleteByUserId(userId: string): Promise<boolean> {
    try {
      ...
      console.log(
        "✅ Connection sessions deleted successfully for user:",
        userId
      );
      return true;
    } catch (error) {
  ...
  async getWithUserInfo(sessionId: string) {
    try {
      ...
      console.error(
        "❌ Failed to get connection session with user info:",
        error
      );
      throw error;
    }
  ...
  async anonymizeOldSessions(daysOld: number = 90): Promise<number> {
    try {
      ...
      console.log(
        `✅ Anonymized ${result.count} sessions older than ${daysOld} days`
      );
      return result.count;
    } catch (error) {
  ...
  async deleteOldSessions(daysOld: number = 365): Promise<number> {
    try {
      ...
      console.log(
        `✅ Deleted ${result.count} sessions older than ${daysOld} days`
      );
      return result.count;
    } catch (error) {
  ...
  async aggregateSessionsForDate(date: Date): Promise<void> {
    try {
      ...
      if (sessions.length === 0) {
        console.log(
          `No sessions to aggregate for ${date.toISOString().split("T")[0]}`
        );
        return;
      }
      ...
      for (const session of sessions) {
        const key = `${session.platform}|${
          session.serverLocation || "unknown"
      ...
      for (const [, group] of groups) {
        const avgDuration = Math.round(
          group.totalDuration / group.totalSessions
        );
      ...
      console.log(
        `✅ Aggregated ${sessions.length} sessions into ${
          groups.size
        } aggregates for ${date.toISOString().split("T")[0]}`
      );
    } catch (error) {
