    ...
    try {
      ...
      if (
        ...
        res.status(400).json({
          success: false,
          error:
            "Missing required fields: sessionToken, transactionId, originalTransactionId, productId",
        } as ApiResponse);
        return;
      }
      ...
      if (receiptData && receiptData.length > 0) {
        ...
        if (receiptResult.status !== 0) {
          ...
          if (!purchase) {
            console.log(
              "⚠️ Transaction not found in receipt, using transaction IDs directly"
            );
          } else {
            environment =
              receiptResult.environment === "Production"
                ? "Production"
                : "Sandbox";
          }
        }
      ...
      const purchaseDate = purchase?.purchase_date_ms
        ? new Date(parseInt(purchase.purchase_date_ms))
        : new Date();
      ...
      console.log(
        "✅ Apple IAP subscription linked successfully:",
        subscription.id
      );
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const subscriptionWithIAP =
        activeSubscription as SubscriptionWithAppleIAP;
      ...
      if (!isActive) {
        ...
        await subscriptionModel.update(activeSubscription.id, {
          status: "inactive",
        });
      }
  ...
  try {
    ...
    for (const purchase of inAppPurchases) {
      ...
      await subscriptionModel.create({
        ...
        appleEnvironment:
          receiptResult.environment === "Sandbox" ? "Sandbox" : "Production",
        status: expiresDate && expiresDate > new Date() ? "active" : "inactive",
        planId: purchase.product_id,
        planName:
          purchase.product_id === "com.keenvpn.premium.annual"
            ? "Premium VPN - Annual"
            : "Premium VPN",
        priceAmount:
          purchase.product_id === "com.keenvpn.premium.annual" ? 130.99 : 0,
        priceCurrency: "USD",
        billingPeriod: "year",
        currentPeriodStart: purchaseDate,
        currentPeriodEnd: expiresDate || undefined,
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const isExpired = subscriptionWithIAP.currentPeriodEnd
        ? subscriptionWithIAP.currentPeriodEnd < now
        : false;
