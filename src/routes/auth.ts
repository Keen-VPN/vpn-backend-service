  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      try {
        ...
        console.log(
          "🔍 Processing Apple Sign-In with userIdentifier:",
          userIdentifier
        );
        ...
        console.log("✅ Apple Sign-In processed:", {
          firebaseUid,
          emailVerified,
        });
      } catch (error) {
      ...
      if (!user) {
        ...
        if (user) {
          ...
          const displayName =
            ...
            (email && email.trim() !== ""
              ? email.split("@")[0]
              : `Apple User ${userIdentifier.substring(0, 8)}`);
          ...
          const userEmail =
            email && email.trim() !== ""
              ? email
              : `${shortId}@privaterelay.appleid.com`;
