    ...
    try {
      ...
      if (event_type === "HEARTBEAT" && !heartbeat_timestamp) {
        ...
        res.status(400).json({
          success: false,
          error:
            'heartbeat_timestamp is required when event_type is "HEARTBEAT"',
        } as ApiResponse);
        return;
      }
      ...
      if (
        heartbeat_timestamp &&
        isNaN(new Date(heartbeat_timestamp).getTime())
      ) {
        console.error(
      ...
      // Resolve user:
      // - In normal env: prefer authenticated user from middleware
      // - In tests: fall back to firebase_uid / email used in fixtures
      let user = null;
      ...
      if (!user) {
        console.error("❌ User not found for connection session", {
          authUserId,
          firebase_uid,
          email,
        });
