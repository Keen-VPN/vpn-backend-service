import type { SalesContactRequest, ValidationResult, ValidationError } from '../types/index.js';
...
function validateWorkEmail(workEmail: string): ValidationError | null {
  ...
  if (!isValidEmail(trimmedEmail)) {
    return { field: 'workEmail', message: 'Please enter a valid email address' };
  }
  
  if (isDisposableEmail(trimmedEmail)) {
    return { field: 'workEmail', message: 'Disposable email addresses are not allowed. Please use your work email.' };
  }
  
  if (isPersonalEmail(trimmedEmail)) {
    return { field: 'workEmail', message: 'Please use your work email address instead of a personal email for enterprise inquiries' };
  }
...
function validateTeamSize(teamSize: number | string): ValidationError | null {
  ...
  if (size > 100000) {
    return { field: 'teamSize', message: 'Team size must be less than 100,000 users' };
  }
  
  // Warn for very small teams (but don't block)
  if (size < 5) {
    return { field: 'teamSize', message: 'Note: Our enterprise plans are typically designed for teams of 5+ users. Consider our individual plans for smaller teams.' };
  }
...
function validateCountryRegion(countryRegion: string): ValidationError | null {
  ...
  if (countryRegion.trim().length < 2) {
    return { field: 'countryRegion', message: 'Please enter a valid country or region' };
  }
  
  if (countryRegion.trim().length > 100) {
    return { field: 'countryRegion', message: 'Country/region must be less than 100 characters' };
  }
...
function validateConsent(hasConsent: boolean): ValidationError | null {
  if (hasConsent !== true) {
    return { field: 'hasConsent', message: 'You must consent to us contacting you about your inquiry' };
  }
 ...
 */
function validateOptionalText(value: string | undefined, fieldName: string, maxLength: number): ValidationError | null {
...
function detectSpamPatterns(data: SalesContactRequest): ValidationError[] {
  ...
  textFields.forEach(text => {
    if (text && /(.)\1{10,}/.test(text)) { // 10+ repeated characters
      spamErrors.push({ 
        field: 'message', 
 ...
 */
export function validateSalesContactRequest(data: SalesContactRequest): ValidationResult {
  ...
  const contactMethodError = validateOptionalText(data.preferredContactMethod, 'preferredContactMethod', 100);
  if (contactMethodError) errors.push(contactMethodError);
  
  const contactTimeError = validateOptionalText(data.preferredContactTime, 'preferredContactTime', 100);
 ...
 */
export function sanitizeSalesContactRequest(data: SalesContactRequest): SalesContactRequest {
  return {
